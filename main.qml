...
Window {
    ...
    flags: Qt.Window | Qt.FramelessWindowHint  // Теперь можно включить!

    // Состояние загрузки
    property bool isLoading: true  // SplashScreen показывается 700ms
    property bool componentsLoaded: true

    // УБРАЛ MouseArea для перетаскивания - он блокирует кнопки!
    // Вместо этого добавлю перетаскивание только на пустую область TopMenuBar
