    ...
    function tryNextOnTrack(track) {
        ...
        if (p !== "") {
            ...
            if (!next) {
                ...
                // FIX: findNextOnTrack ищет клипы НАЧИНАЮЩИЕСЯ после currentTime.
                // Но другая дорожка может быть сейчас В СЕРЕДИНЕ клипа —
                // clipPathAt проверяет наличие клипа прямо сейчас.
                ...
                // Иначе ждём — другая дорожка ещё играет или скоро будет
            }
